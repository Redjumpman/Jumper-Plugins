...
class Animelist:
    ...
    async def fetch_info(self, ctx, cmd, title):
        ...
        try:
            ...
            msg += 'http://myanimelist.net/{}/{}'.format("anime", entry.find('id').text)

            await self.bot.say(msg)
        except:
            await self.bot.say("Your username or password is not correct.You need to create an account on myanimelist.net.\nIf you have an account use **{}animeset** to set your credentials".format(ctx.prefix))
